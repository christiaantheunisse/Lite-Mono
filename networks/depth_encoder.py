...
class LiteMono(nn.Module):
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
                 global_block=[1, 1, 1], global_block_type=['LGFI', 'LGFI'],
                 drop_path_rate=0.2, layer_scale_init_value=1e-6, expan_ratio=6,
                 heads=[8, 8], use_pos_embd_xca=[True, False], **kwargs):

        super().__init__()

        if model == 'lite-mono':
            # self.num_ch_enc = np.array([48, 80, 128])
            self.num_ch_enc = np.array([48, 80])
            # self.depth = [4, 4, 10]
            self.depth = [4, 10] # I have to find out if this works
            # self.dims = [48, 80, 128]
            self.dims = [48, 80]
            ...
            print("Sorry, this option isn't available on this branch")
            raise NotImplementedError
